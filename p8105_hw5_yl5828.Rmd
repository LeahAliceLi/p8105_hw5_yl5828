...
#### Comment:
Power is about 0.05 when mu = 0, which is just the type I error rate. As the true mean moves farther from 0 and become larger effect size, power increases rapidly and approaches 1 by μ around 5–6. So larger effect sizes leads to higher power.
...
#### Comment:
The solid line (all samples): Falls perfectly on the 45° diagonal line, showing that mu hat is an unbiased estimator of μ across all samples
...
For large mu, almost all tests are significant, so conditioning on rejection doesn’t change much, and the dotted line comes back close to the solid line.
The sample average of 𝜇̂ across tests for which the null is rejected is usually larger than the true value of 𝜇, since 𝜇̂is an unbiased estimator of mu.
